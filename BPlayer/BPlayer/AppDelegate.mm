...
- (void)createDBTable{
    ...
    //创建音乐表
    BOOL musicBool=[CoreFMDB executeUpdate:@"create table if not exists music(id integer primary key autoIncrement,server text,uri text,title text,size integer,artist text,album text,composer text,genres text,date text,duration double,format text);"];
    if(musicBool){
